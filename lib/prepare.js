...
module.exports = async (pluginConfig, context) => {
  if ("placeholderRegExp" in pluginConfig) {
    if (
      typeof pluginConfig.placeholderRegExp === "string" ||
      pluginConfig.placeholderRegExp instanceof String
    ) {
  ...
  await glob(fileGlob).then(async (files) => {
    ...
    await Promise.all(
      await files.map(async (filename) => {
        ...
        logger.log(`File, ${filename}, has been written`);
        return filename;
      })
    );
  });
